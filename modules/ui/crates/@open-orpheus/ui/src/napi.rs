...
fn resolve_napi_get_uv_event_loop() -> Result<NapiGetUvEventLoop, String> {
  ...
  unsafe {
    ...
    if handle.is_null() {
      ...
      return Err(format!("failed to open current process for symbol lookup: {msg}"));
    }

    let symbol = dlsym(handle, b"napi_get_uv_event_loop\0".as_ptr().cast::<c_char>());
    if symbol.is_null() {
      let err = dlerror();
      let msg = if err.is_null() {
    ...
    return Ok(std::mem::transmute::<*mut c_void, NapiGetUvEventLoop>(symbol));
  }
...
fn napi_get_uv_event_loop_fn() -> Result<NapiGetUvEventLoop, String> {
  ...
  RESOLVED
    .get_or_init(resolve_napi_get_uv_event_loop)
    .clone()
}
